package language.project.convertSima ;
	...
	class ExtractBlocs
	{
		...
		public static function extractClassFunctions(_oSClass:SClass):Void {
			...
			//Debug.fTrace("--Class Function analyse : " + oCurrSClass.sName);
			...
			for (i in 0 ..._i) {
				...
				if(!_oFunction.bNoLine){
					if(_oFunction.eFuncType != EuFuncType.Extend){ //Do make empty extend function list
						//Debug.fTrace("--Function analyse : " + _oFunction.sName);
						allLineInFunction(_oSClass,  _oFunction);
					}
				}
			}
			/*
			if (_oSClass.oFuncDestrutor != null) { //special case for destructor ///--> now added in _aFunctionList
				allLineInFunction(_oSClass,  _oSClass.oFuncDestrutor);
			}*/
			
			fExtractBetweenFunction(_oSClass);
			
			if (_oSClass.bHaveExtend && !_oSClass.oFuncConstructor.bCallExtendConstuctor && !_oSClass.bDefaultConstrutorGenereted && !_oSClass.oPackage.oSFrame.bSkipContent){

				//Debug.fError("Extended constructor not called " + _oSClass.oFuncConstructor.oSClass.sConstructLineToExtract ); 
				//Extendeded constructor call autogenerated!
				LoadVarContent.fSetDefaultConstructLineToExtract(_oSClass.oFuncConstructor, _oSClass.aExtendClass[0]);
				LoadVarContent.fLoadDefaultConstructorLine(_oSClass);
			}
		...
		*/
		...
		public static function fExtractBetweenFunction(_oSClass : SClass):Void {
			...
			for (i in 0 ..._i) {
				var _oFunction : SFunction = _aFunctionList[i];
				if(!_oFunction.bDefaultConstructor && !_oFunction.bFuncGenerated){
					var _nLastLine : UInt = _nTotalLine;
					while (i + 1 < _i) {
						var _oNextFunction : SFunction =  cast(_aFunctionList[i + 1], SFunction);
						if(!_oNextFunction.bDefaultConstructor && !_oNextFunction.bFuncGenerated) { //Or any genreated function
